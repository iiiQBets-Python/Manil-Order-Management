...
document.getElementById('client_id').addEventListener('change', function () {
  ...
  var shipping_add = selectedOption.getAttribute('shipping_add');
  var city = selectedOption.getAttribute('city');
  var state = selectedOption.getAttribute('state');
  var pincode = selectedOption.getAttribute('pincode');

  if (shipping_add) {
    document.getElementById('shipping_address').value = shipping_add;
  } else {
    document.getElementById('shipping_address').value = null;
  }
  ...
  var gst_typeInp = document.querySelectorAll('.gst_type');
  ...
  if (state) {
    ...
    document.getElementById('shipping_state').value = null;
  }
  ...
  if (pincode) {
    document.getElementById('shipping_pin').value = pincode;
  } else {
    document.getElementById('shipping_pin').value = null;
  }


  var client_id_value = this.value;
  
  var po_authoritySelect = document.getElementById('po_authority');
  var options = po_authoritySelect.querySelectorAll('option');

  options.forEach(option => {      
    if (option.value === "") {
        option.style.display = "block"; // Keep the default empty option visible
    } else if (option.dataset.clientid === client_id_value) {
        option.style.display = "block"; // Show options that match the selected client_id
    } else {
        option.style.display = "none"; // Hide options that don't match the selected client_id
    }
  });

  // Enable or disable the po_authority dropdown based on client selection
  po_authoritySelect.disabled = !client_id_value;  
  po_authoritySelect.value = ""; // Reset the selection of PO Authority


  var materialSelects = document.querySelectorAll('.material-select');

  // Loop over each material dropdown and filter its options based on client_id
  materialSelects.forEach(function(selectElement) {
    var options = selectElement.querySelectorAll('option');
    options.forEach(function(option) {
      var clientId = option.getAttribute('data-clientid');

      // Show option if client_id matches or if it's the default disabled option
      if (!clientId || clientId === client_id_value) {
          option.style.display = "block"; // Show the option
      } else {
          option.style.display = "none";  // Hide the option
      }
    });
    // Reset the selection after filtering
...
function handle_material(value) {
  ...
  var costunit = selectedOption.getAttribute('cost_unit');
  ...
  if (costunit) {
    document.getElementById(`base_price_${value}`).value = costunit;
  } else {
